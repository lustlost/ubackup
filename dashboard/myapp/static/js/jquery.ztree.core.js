...
(function($){
	...
	_initNode = function(setting, level, n, parentNode, isFirstNode, isLastNode, openFlag) {
		...
		n.getStringPath = function () {
			var node_path = '';
			var node_paths = data.getNodePath(setting, n);
			for (var i in node_paths){
				node_path+='/'+node_paths[i].name;
			}
			return node_path;
		};
		n.isAjaxing = false;
	...
	data = {
		...
		getNodePathName: function(setting, node) {
			if (!node) return null;

			var path;
			if(node.parentTId) {

				path = node.getParentNode().getPath()[0].name;
			} else {
				//path = [];
				path='';
			}
